...
C_vapor = 16; % Assumed f*Re constant for laminar vapor flow
dP_v = (C_vapor * mu_v * Q_in * L_eff) / (rho_v * A_vapor * d_h_vapor^2 * h_fg); % Vapor viscous loss
g = 9.81;
...
Q_max = (dP_cap - dP_g) / ...
        ( (mu_l*(L_eff/2))/(rho_l*A_wick_cond*K_cond*h_fg) + ...
          (mu_l*(L_eff/2))/(rho_l*A_wick_evap*K_evap*h_fg) + ...
          (C_vapor*mu_v*L_eff)/(rho_v*A_vapor*d_h_vapor^2*h_fg) );
...
k_wick_evap = k_shell * ((k_shell + k_l - (1-epsilon_evap)*(k_shell-k_l)) / (k_shell + k_l + (1-epsilon_evap)*(k_shell-k_l)));
k_wick_cond = k_shell * ((k_shell + k_l - (1-epsilon_cond)*(k_shell-k_l)) / (k_shell + k_l + (1-epsilon_cond)*(k_shell-k_l)));
